...
import { Play, Pause, RotateCcw, Save, Zap, Coffee, Moon, Loader2, Settings2, BellOff, Volume2 } from 'lucide-react';
...
const WorkTimer: React.FC<WorkTimerProps> = ({ lang, userId }) => {
  ...
  const [totalTime, setTotalTime] = useState(presets[mode]); // To track the initial set time for progress
  ...
  return (
    <div className={`max-w-4xl mx-auto space-y-12 animate-in fade-in duration-700 pb-20 ${isAlarmPlaying ? 'animate-pulse' : ''}`}>
      <div className="text-center space-y-4">
        <h2 className="text-5xl font-black text-slate-900 tracking-tight">{t.timer_title}</h2>
      ...
      <div className={`bg-white/80 backdrop-blur-3xl rounded-[64px] border ${isAlarmPlaying ? 'border-rose-500 shadow-rose-200' : 'border-blue-50 shadow-blue-200'} shadow-2xl p-8 lg:p-16 flex flex-col items-center gap-10 relative overflow-hidden transition-colors duration-500`}>
        {/* Progress Background */}
        ...
        <div className="w-full flex flex-col md:flex-row items-center justify-center gap-4 relative z-10">
          ...
          {mode === 'custom' && (
            <div className="flex items-center gap-3 bg-white border border-blue-100 px-4 py-2 rounded-2xl shadow-sm animate-in zoom-in duration-300">
               <input 
                 type="number" 
                 value={customMinutes} 
                 onChange={handleCustomTimeChange}
                 className="w-16 bg-transparent border-none font-black text-blue-600 text-center text-lg outline-none"
                 min="1"
                 max="999"
               />
        ...
        <div className="relative group">
          ...
          <svg className="w-64 h-64 md:w-80 md:h-80 transform -rotate-90 relative">
            ...
            <circle
              ...
              strokeLinecap="round"
              className={`${isAlarmPlaying ? 'text-rose-600' : (mode === 'focus' || mode === 'custom' ? 'text-blue-600' : 'text-emerald-500')} transition-all duration-1000`}
              pathLength="100"
            />
          </svg>
          <div className="absolute inset-0 flex items-center justify-center">
            <span className={`text-7xl md:text-8xl font-black tabular-nums tracking-tighter ${isAlarmPlaying ? 'text-rose-600 animate-pulse' : 'text-slate-900'}`}>
              {formatTime(timeLeft)}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-8 relative z-10">
          <button 
            onClick={resetTimer}
            className="w-14 h-14 rounded-2xl bg-slate-100 text-slate-400 hover:bg-slate-200 hover:text-slate-600 flex items-center justify-center transition-all active:scale-90"
            title={t.reset}
          >
            <RotateCcw size={24} />
          </button>
          ...
          <button 
            onClick={isAlarmPlaying ? stopAlarm : toggleTimer}
            className={`w-28 h-28 rounded-[40px] flex items-center justify-center shadow-2xl transition-all active:scale-95 ${isAlarmPlaying ? 'bg-rose-600 text-white animate-bounce' : (isActive ? 'bg-slate-900 text-white' : 'blue-btn text-white')}`}
          >
      ...
      {/* Log Session Modal */}
      {sessionCompleted && (
        <div className="bg-white p-10 rounded-[48px] border-2 border-blue-500 shadow-2xl animate-in zoom-in duration-500 space-y-6 relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 relative z-10">
            ...
            <div>
              <h3 className="text-3xl font-black text-slate-900">{lang === 'bn' ? 'সেশন সম্পন্ন!' : 'Session Complete!'}</h3>
          ...
          <div className="space-y-4 relative z-10">
            <input 
              ...
              placeholder={lang === 'bn' ? "কাজের শিরোনাম..." : "What were you working on?"}
              className="w-full bg-slate-50 border-none rounded-3xl py-6 px-8 font-black text-lg outline-none focus:ring-4 focus:ring-blue-500/10 transition-all"
            />
            <div className="flex gap-4 pt-2">
              <button 
                onClick={() => setSessionCompleted(false)}
                className="flex-1 py-5 rounded-[24px] font-black text-slate-400 hover:bg-slate-100 transition-all uppercase tracking-widest text-[10px]"
              >
                {t.cancel}
              </button>
              <button 
                onClick={logSession}
                disabled={isSaving}
                className="flex-1 blue-btn text-white py-5 rounded-[24px] font-black flex items-center justify-center gap-3 shadow-xl disabled:opacity-50 transition-all"
              >
